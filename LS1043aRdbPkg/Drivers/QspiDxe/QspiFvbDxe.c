    ...
    )
{
  ...
  ASSERT((PcdGet32(PcdFlashNvStorageVariableSize) > 0) &&
      (PcdGet32(PcdFlashNvStorageVariableSize) / Instance->Media.BlockSize 
       > 0));
  ASSERT((PcdGet32(PcdFlashNvStorageFtwWorkingSize) > 0) &&
      (PcdGet32(PcdFlashNvStorageFtwWorkingSize) / Instance->Media.BlockSize 
       > 0));
  ...
  // Install the combined super-header in the Qspi
    ...
    )
{
  ...
  if( CompareGuid (
        &FwVolHeader->FileSystemGuid, &gEfiSystemNvDataFvGuid
        ) == FALSE ) {
...
/**
  ...
  desired firmware volume settings. On successful
  return, it contains the new settings of
  the firmware volume.
  Type EFI_FVB_ATTRIBUTES_2 is defined in
  EFI_FIRMWARE_VOLUME_HEADER.
    ...
    )
{
  ...
  DEBUG ((DEBUG_BLKIO, "%a(Lba=%ld, BlockSize=0x%x, LastBlock=%ld)\n",
        __FUNCTION__,Lba, Instance->Media.BlockSize,
        Instance->Media.LastBlock));

  if (Lba > Instance->Media.LastBlock) {
    DEBUG ((EFI_D_ERROR, "%a: ERROR - Parameter LBA %ld is beyond the" 
          "last Lba (%ld).\n",__FUNCTION__,
          Lba, Instance->Media.LastBlock));
    Status = EFI_INVALID_PARAMETER;
  } else {
    // This is easy because in this platform each Qspi device has equal sized 
    // blocks.
    ...
    )
{
  ...
  DEBUG ((DEBUG_BLKIO, "%a(Parameters: Lba=%ld, Offset=0x%x, *NumBytes=0x%x"
       ", Buffer @ 0x%08x)\n",__FUNCTION__, Instance->StartLba + Lba, Offset,
       *NumBytes, Buffer));
  ...
  if ((Instance->StartLba + Lba) > Instance->Media.LastBlock) {
    DEBUG ((EFI_D_ERROR, "FvbRead: ERROR - Parameter LBA %ld is beyond the last"
          "Lba (%ld).\n", Lba, Instance->Media.LastBlock));
  ...
  return QspiRead (Instance, Instance->StartLba + Lba, Offset, *NumBytes,
                    Buffer);
}
    ...
    )
{
  ...
  if ((Instance->StartLba + Lba) > Instance->Media.LastBlock) {
    DEBUG ((EFI_D_ERROR, "FvbWrite: ERROR - Parameter LBA %ld is beyond the"
          "last Lba (%ld).\n", Lba, Instance->Media.LastBlock));
    return EFI_INVALID_PARAMETER;
  }
  ...
  return QspiWrite (Instance, Instance->StartLba + Lba, Offset, *NumBytes,
                    Buffer);
}
...
/**
  ...
  @retval EFI_DEVICE_ERROR    The block device is not functioning correctly and
                              could not be written.The firmware device may have
    ...
    )
{
  ...
  if (Instance->Media.ReadOnly == TRUE) {
    ...
    DEBUG ((EFI_D_ERROR, "FvbEraseBlocks: ERROR - Device is in WriteDisabled"
          "state.\n"));
    return EFI_ACCESS_DENIED;
  }

  // Before erasing, check the entire list of parameters to ensure all 
  // specified blocks are valid
  ...
  do {
    ...
    DEBUG ((DEBUG_BLKIO, "%a: Check if: ( StartingLba=%ld + NumOfLba=%d - 1 ) >"
          "LastBlock=%ld.\n",__FUNCTION__, Instance->StartLba + StartingLba,
  ...
  do {
    ...
    while (NumOfLba > 0) {
      ...
      BlockOffset = GET_BLOCK_OFFSET(Instance->StartLba + StartingLba);

      // Erase it
      DEBUG ((DEBUG_BLKIO, "%a: Erasing Lba=%ld @ Offset 0x%08x.\n",
            __FUNCTION__, Instance->StartLba + StartingLba, BlockOffset));
      Status = QspiFlashErase(BlockOffset, Instance->Media.BlockSize);
      if (EFI_ERROR(Status)) {
        VA_END (Args);
    ...
    )
{
  ...
  DEBUG((DEBUG_BLKIO,"QspiFvbInitialize\n"));
  ...
  if (BootMode == BOOT_WITH_DEFAULT_SETTINGS) {
    ...
    // Determine if there is a valid header at the beginning of the Qspi
  ...
  if (EFI_ERROR(Status)) {
    ...
    // Erase all the Qspi that is reserved for variable storage
  ...
  // Note: all the Qspi Flash region needs to be reserved into the UEFI Runtime 
  //       memory; even if we only use the small block region at the top of the 
  //       Qspi Flash.
  //       The reason is when the Qspi Flash memory is set into program mode, 
  //       the command is written as the base of the flash region 
  //       (ie: Instance->DeviceBaseAddress)
