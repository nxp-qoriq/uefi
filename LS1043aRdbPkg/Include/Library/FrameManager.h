...
C_ASSERT(FMAN_FREE_POOL_SIZE % FMAN_FREE_POOL_ALIGNMENT == 0);
...
#define FMAN_MAX_NUM_RX_1G_PORTS    	NUM_FMAN1_DTSEC

#define FMAN_RX_10G_PORT_ID_BASE    	0x10

#define FMAN_TX_1G_PORT_ID_BASE     	0x28
#define FMAN_MAX_NUM_TX_1G_PORTS    	NUM_FMAN1_DTSEC
...
C_ASSERT(sizeof(FMAN_BMI_COMMON) == 1024);
C_ASSERT(OFFSET_OF(FMAN_BMI_COMMON, Gde) == 0x100);
...
#else
  struct {
    UINT8           FmanDtesc[4*1024];
    FMAN_MDIO       FmanMdio;
  } Mac1g[8];   /* support up to 8 1g controllers */

  struct {
    UINT8           Fman10gec[4*1024];
    UINT8           Fman10gecMdio[4*1024];
  } Mac10g[1];

  UINT8             Reserved4[48*1024];
#endif
...
C_ASSERT(sizeof(FMAN_CCSR) == 0x100000);
C_ASSERT(OFFSET_OF(FMAN_CCSR, Policer) == (512 + 256) * 1024);
...
C_ASSERT(sizeof(FMAN_FIRMWARE) == OFFSET_OF(FMAN_FIRMWARE, MicrocodeHeaders));
