  ...
  )
{
  ...
  if (AML_RD_IS_LARGE (CurrRdElement)) {
    // The stream must contain at least the size of
    // a large resource data element header.
    if (AmlStreamGetFreeSpace (FStream) < AML_RD_LARGE_HEADER_SIZE) {
      return 0;
    }
    return ((AML_RD_LARGE*)CurrRdElement)->Length +
             AML_RD_LARGE_HEADER_SIZE;
  }

  // Header is a small resource data element.
  return (((AML_RD_SMALL*)CurrRdElement)->Id &
  ...
  )
{
  ...
  while (TRUE) {
    ...
    if (*CurrRdElement == AML_RD_BUILD_SMALL_DESC_ID (EAmlRdsIdEndTag)) {
