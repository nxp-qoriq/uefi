/** SataController.c
  ...
  Reference taken from SataControllerDxe driver from
  OvmfPkg/SataControllerDxe/
  ...
  )
{
  ...
  DEBUG ((EFI_D_INFO, "SATA device CLASS: %x SUBCLASS : %x \n", PciClass, PciSubClass));
  //
  // Examine Ide PCI Configuration table fields
  //
  if ((PciClass != PCI_CLASS_MASS_STORAGE) || (PciSubClass != PCI_CLASS_MASS_STORAGE_SATADPA)) {
    return EFI_UNSUPPORTED;
  }
  ...
  )
{
  ...
  if (IS_PCI_IDE (&PciData)) {
    DEBUG ((EFI_D_INFO, "SataController Is in IDE MODE !\n"));
  ...
  // configuring Physical Control Layer parameters
  // for Port 0
  AhciWriteReg(PciIo, SATA_PPCFG, CONFIG_PORT_PHYSICAL);
  // This register controls the configuration of the
  // Transport Layer for  Port 0
  if (PcdGetBool(PcdSataErratumA009185))
    AhciWriteReg(PciIo, SATA_PTC, CONFIG_PORT_RXWM);
  else
    AhciWriteReg(PciIo, SATA_PTC, CONFIG_PORT_TRANSPORT);
  
  DEBUG ((EFI_D_RELEASE, "SataController is in AHCI MODE, Port configuration done!\n"));
  ...
  DEBUG ((EFI_D_RELEASE, "IDE Controller Init Protocol Installed successfully !\n"));
