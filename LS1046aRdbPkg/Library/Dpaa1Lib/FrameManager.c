...
{ 
  FMAN_CCSR *FmanRegs = (FMAN_CCSR *)FMAN_ADDR;
  ...
  )
{
  ...
  INT32 Num;
  UINTN Base, PhyRegs;
  FMAN_CCSR *Reg = (FMAN_CCSR *)FMAN_ADDR;
  ...
  )
{
  ...
  MmioWriteBe32((UINTN)&Pram->Mode, PRAM_MODE_GLOBAL);
  ...
  for (I = 0; I < RX_RING_SIZE; I++) {
    ...
    BufHi = Upper32Bits(VirtToPhys(RxBufPool +
			I * MAX_RXBUF_LEN));
    BufLo = Lower32Bits(VirtToPhys(RxBufPool +
			I * MAX_RXBUF_LEN));
  ...
  BdRingBaseHi = Upper32Bits(VirtToPhys(RxRingBase));
  BdRingBaseLo = Lower32Bits(VirtToPhys(RxRingBase));
  ...
  )
{
  ...
  TxQd = &Pram->Txqd;
  BdRingBaseHi = Upper32Bits(VirtToPhys(TxRingBase));
  BdRingBaseLo = Lower32Bits(VirtToPhys(TxRingBase));
...
FmanInitFPM(FMAN_FPM *Fpm)
{
  ...
  //
  for(Index = 0; Index < ARRAY_NUM_ENTRIES(Fpm->Cev); Index++) {
  ...
  )
{
  ...
  SET_BIT_FIELD(RegValue, BMI_CFG1_FBPO_MASK, BMI_CFG1_FBPO_SHIFT,
                FreeBufferPoolOffsetInBlocks);
  SET_BIT_FIELD(RegValue, BMI_CFG1_FBPS_MASK, BMI_CFG1_FBPS_SHIFT,
  ...
  for(Index = 0; Index < FMAN_MAX_NUM_RX_1G_PORTS; Index++) {
    ...
    RegValue = 0;
    SET_BIT_FIELD(RegValue, BMI_PP_MXT_MASK, BMI_PP_MXT_SHIFT, 4 - 1);
    MmioWriteBe32((UINTN)&Bmi->Pp[PortIndex], RegValue);
    RegValue = 0;
    SET_BIT_FIELD(RegValue, BMI_PFS_IFSZ_MASK, BMI_PFS_IFSZ_SHIFT, 0xf);
  ...
  RegValue = 0;
  SET_BIT_FIELD(RegValue, BMI_PP_MXT_MASK, BMI_PP_MXT_SHIFT, 12 - 1);
  SET_BIT_FIELD(RegValue, BMI_PP_MXD_MASK, BMI_PP_MXD_SHIFT, 3 - 1);
  MmioWriteBe32((UINTN)&Bmi->Pp[PortIndex], RegValue);
  RegValue = 0;
  SET_BIT_FIELD(RegValue, BMI_PFS_IFSZ_MASK, BMI_PFS_IFSZ_SHIFT, 0xf);
  MmioWriteBe32((UINTN)&Bmi->Pfs[PortIndex], RegValue);
  ...
  )
{
  ...
  /* setup TxBD */
  MemWriteMasked(&Txbd->BufPtrHi, (UINT16)Upper32Bits(VirtToPhys(Data)));
  MmioWriteBe32((UINTN)&Txbd->BufPtrLo, Lower32Bits(VirtToPhys(Data)));
  ...
  )
{
  ...
  // Clear DMA status:
  //
  MmioSetBitsBe32(FmanRegs->FmanDma.Sr, DMA_SR_ALL_ERRORS);

  //
  // Configure DMA to stop at bus error event on external bus
  //
  MmioSetBitsBe32(FmanRegs->FmanDma.Mr, DMA_MR_SBER);
