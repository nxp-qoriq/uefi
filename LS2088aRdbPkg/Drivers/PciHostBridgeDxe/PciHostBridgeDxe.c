...
#define PCI4_MEM64_BASE     FixedPcdGet64 (PcdPci4Mmio64Base)
...
//
UINTN RootBridgeNumber[4] = { 1, 1, 1, 1 };
...
PCI_HOST_BRIDGE_INSTANCE    *HostBridge[4];
PCI_ROOT_BRIDGE_INSTANCE    *PrivateData[4];
  ...
  )
{
  ...
  switch(PcieNum){
	...
	case 3:
		return(IsSerDesLaneProtocolConfigured(SerDes1ProtocolMap, SerDes2ProtocolMap, PCIE4));
	default:
  ...
  )
{
  ...
  for (Loop1 = 0; Loop1 < HOST_BRIDGE_NUMBER; Loop1++) {
    ...
    if (EFI_ERROR (Status)) {
      ...
      DEBUG ((EFI_D_INFO, "%a: Successfully installed resource alloc protocol\n", __FUNCTION__));
    }   
    ...
    for (Loop2 = 0; Loop2 < HostBridge[Loop1]->RootBridgeNumber[Loop1]; Loop2++) {
      ...
      if (Loop1 == 4) {
        PrivateData[Loop1]->DevicePath.AcpiDevicePath.UID = 3;
        PrivateData[Loop1]->PciMemBase64 = PCI4_MEM64_BASE;
      }
  ...
  Status = AddMemorySpace(PCI4_MEM64_BASE, PCI_MEM64_SIZE);
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "Memory Resource couldn't be added for PCI4_MEM64\n"));
    return EFI_DEVICE_ERROR;
  }
  ...
  )
{
  ...
  if (Configuration == NULL) {
    DEBUG((EFI_D_ERROR, "PciHbRaSubmitResources(): NULL Configuration\n"));
    return EFI_INVALID_PARAMETER;
  }
