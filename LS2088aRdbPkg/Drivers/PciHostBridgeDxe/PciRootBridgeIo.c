  ...
  )
{
  ...
  if (HostNo == 1) {
    ...
    PrivateData->PciBaseAddress64 = PcdGet64 (PcdPci4ExpressBaseAddress);
    if (!PrivateData->PciBaseAddress64) {
      DEBUG ((EFI_D_ERROR, "%a: PCI 4 host bridge not present\n", __FUNCTION__));
      return EFI_ABORTED;
    }
  ...
  if (FeaturePcdGet(PcdPciDebug) == TRUE)
    DEBUG ((EFI_D_INFO, "PCI : Located MetronomeArch protocol\n"));
  ...
  )
{
  ...
  *Mapping = NULL;
  ...
  if ((*NumberOfBytes) > 0x40000000ULL) {
    DEBUG((EFI_D_ERROR, "Cannot map more than 1G at one transaction\n"));
    return EFI_INVALID_PARAMETER;
  }
  ...
  if ((PhysicalAddress + *NumberOfBytes) > 0x40000000ULL) {
    ...
    *Mapping = MapInfo;
    ...
    MapInfo->MappedHostAddress = 0x0000ffffffffffff;

    //
    // Allocate a buffer in 40bit memory to map the transfer to.
    //
  ...
  )
{
  ...
  PhysicalAddress = (EFI_PHYSICAL_ADDRESS)(0xffffffffffff);
