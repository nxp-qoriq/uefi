...
{
  VOID * BaseAddress = (VOID *)I2C1_BASE_ADDRESS;
  ...
  )
{
  ...
  for (Count = 0; Count < RequestPacket->OperationCount; Count++) {
    ...
    if (Flag == I2C_READ_FLAG) {
      Ret = I2cDataRead ((VOID *)I2C1_BASE_ADDRESS, SlaveAddress,
      ...
      Ret = I2cDataWrite ((VOID *)I2C1_BASE_ADDRESS, SlaveAddress,
              0x00, sizeof(SlaveAddress)/8, Buffer, Length);
      if (Ret != EFI_SUCCESS) {
