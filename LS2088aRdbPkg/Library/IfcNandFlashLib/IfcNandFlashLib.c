...
INTN IfcRunCmd()
{
	...
	MmioWrite32((UINTN)&gNandFlashInfo->IfcRegs->ifc_nand.nand_csel,
			(NandCs << 26));

	/* start read/write seq */
	MmioWrite32((UINTN)&gNandFlashInfo->IfcRegs->ifc_nand.nandseq_strt,
	...
	MmioWrite32((UINTN)&gNandFlashInfo->IfcRegs->ifc_nand.nand_evter_stat,
			Status);
...
{
	/* Program ROW0/COL0 */
	MmioWrite32((UINTN)&gNandFlashInfo->IfcRegs->ifc_nand.row0, page_addr);
	MmioWrite32((UINTN)&gNandFlashInfo->IfcRegs->ifc_nand.col0, column);
}
		...
		)
{
	...
	MmioWrite32((UINTN)&gNandFlashInfo->IfcRegs->ifc_nand.nand_fir0,
		  (FSL_IFC_FIR_OP_CW0 << FSL_IFC_NAND_FIR0_OP0_SHIFT) |
		  (FSL_IFC_FIR_OP_RDSTAT << FSL_IFC_NAND_FIR0_OP1_SHIFT));
	MmioWrite32((UINTN)&gNandFlashInfo->IfcRegs->ifc_nand.nand_fcr0, IFC_NAND_CMD_STATUS <<
		  FSL_IFC_NAND_FCR0_CMD0_SHIFT);
	MmioWrite32((UINTN)&gNandFlashInfo->IfcRegs->ifc_nand.nand_fbcr, 1);
	SetAddressRegs(0, 0);
...
)
{
  ...
  for (Index = 0;
	...
	gNandFlashInfo->PageSize = NAND_PAGE_SIZE_4K;
	gNandFlashInfo->SparePageSize = NAND_SPARE_AREA_SIZE_224B;
	gNandFlashInfo->BlockSize = NAND_BLOCK_SIZE_512K;
	Found = TRUE;
	break;
    }
  }
  ...
  DEBUG((EFI_D_RELEASE, "NAND Flash (Manufacturer Id:0x%x, Device Id:0x%x) detected\n",
				gNandFlashInfo->ManufactureId, gNandFlashInfo->DeviceId));
...
)
{
  ...
  MmioWrite32((UINTN)&gNandFlashInfo->IfcRegs->ifc_nand.nand_evter_en,
                  FSL_IFC_NAND_EVTER_EN_OPC_EN |
                  FSL_IFC_NAND_EVTER_EN_PGRDCMPL_EN |
                  FSL_IFC_NAND_EVTER_EN_FTOER_EN |
                  FSL_IFC_NAND_EVTER_EN_WPER_EN);

	MmioWrite32((UINTN)&gNandFlashInfo->IfcRegs->ifc_nand.ncfgr, 0x0);
...
)
{
  ...
  //Reset NAND part
  Status = IfcNandFlashReset(NULL, FALSE);
  if (EFI_ERROR(Status)) {
    DEBUG((
	EFI_D_ERROR, "Nand reset failed: Status: %x\n", Status));
    return Status;
  }
