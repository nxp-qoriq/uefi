...
static inline
VOID
McWriteCommand(volatile DPAA2_MC_COMMAND *Portal,
...
static inline
MC_CMD_STATUS
McReadResponse(volatile DPAA2_MC_COMMAND *Portal,
	        ...
	        DPAA2_MC_COMMAND *Cmd)
{
  ...
  if (gDpaa2DebugFlags & DPAA2_DEBUG_TRACE_MC_COMMANDS) {
    DPAA2_DEBUG_MSG("Sending MC command: 0x%lx (Cmd ID 0x%x, Token 0x%x)\n",
  ...
  if (Timeout == 0) {
    DPAA2_ERROR_MSG("Timeout waiting for MC response\n");
    Status = EFI_TIMEOUT;
    goto CommonExit;
  }

  if (gDpaa2DebugFlags & DPAA2_DEBUG_TRACE_MC_COMMANDS) {
    DPAA2_DEBUG_MSG("Received MC response: 0x%lx (MC status: 0x%x)\n",
