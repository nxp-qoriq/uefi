 ...
 */
typedef struct _DPAA2_FRAME_ANNOTATION_STATUS {
  UINT8 Reserved;
  UINT8 Ppid;
  UINT16 Ifpid;
  UINT32 Status;
} DPAA2_FRAME_ANNOTATION_STATUS;
  ...
  )
{
  ...
  McFlibError = dprc_create_container(ParentDprcMcIo, MC_CMD_NO_FLAGS,
			              ...
			              &DprcMcPortalOffset);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dprc_create_container() for 0x%p\n",
                    Dpaa2NetInterface);
  ...
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dprc_open() for 0x%p\n",
                    Dpaa2NetInterface);
  ...
  ASSERT(Dpaa2NetInterface->DprcHandle != 0);
  DPAA2_DEBUG_MSG("Created dprc.%u (MC portal physical address 0x%llx) in the MC\n",
  ...
  )
{
  ...
  McFlibError = dprc_close(&Dpaa2NetInterface->DprcMcIo, MC_CMD_NO_FLAGS,
                           Dpaa2NetInterface->DprcHandle);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dprc_close() for DPAA2 network interface 0x%p\n",
                    Dpaa2NetInterface);
  }
  ...
  McFlibError = dprc_destroy_container(ParentDprcMcIo, MC_CMD_NO_FLAGS,
                                       ...
                                       Dpaa2NetInterface->DprcId);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dprc_destroy_container() for DPAA2 network interface 0x%p\n",
                    Dpaa2NetInterface);
  }

  DPAA2_DEBUG_MSG("Destroyed dprc.%u\n", Dpaa2NetInterface->DprcId);
}
  ...
  )
{
  ...
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpbp_create() for 0x%p\n",
                    Dpaa2NetInterface);
    return EFI_DEVICE_ERROR;
  }
  ...
  McFlibError = dpbp_get_attributes(DprcMcIo, MC_CMD_NO_FLAGS,
				    ...
				    &DpbpAttr);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpbp_get_attributes() for 0x%p\n",
                    Dpaa2NetInterface);
    Status = EFI_DEVICE_ERROR;
  ...
  DPAA2_DEBUG_MSG("Created dpbp.%u (HW buffer pool id %u) in the MC\n",
  ...
  )
{
  ...
  McFlibError = dpbp_destroy(DprcMcIo, MC_CMD_NO_FLAGS,
                             Dpaa2NetInterface->DpbpHandle);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpbp_destroy() for DPAA2 network interface 0x%p\n",
                    Dpaa2NetInterface);
  }

  Dpaa2NetInterface->DpbpHandle = 0;
  DPAA2_DEBUG_MSG("Destroyed dpbp.%u\n", Dpaa2NetInterface->DpbpId);
}
  ...
  )
{
  ...
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpio_create() for 0x%p\n",
                    Dpaa2NetInterface);
    return EFI_DEVICE_ERROR;
  }
  ...
  McFlibError = dpio_get_attributes(DprcMcIo, MC_CMD_NO_FLAGS,
				    Dpaa2NetInterface->DpioHandle,
                                    &DpioAttr);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpio_get_attributes() for 0x%p\n",
                    Dpaa2NetInterface);
    Status = EFI_DEVICE_ERROR;
  ...
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpio_enable() for 0x%p\n",
                    Dpaa2NetInterface);
    Status = EFI_DEVICE_ERROR;
    goto ErrorExitDestroyDpio;
  }
  ...
  DPAA2_DEBUG_MSG("Created dpio.%u in the MC ("
  ...
  )
{
  ...
  if (McFlibError != 0) {
      DPAA2_ERROR_MSG("Error calling dpio_disable() for 0x%p\n",
                      Dpaa2NetInterface);
  }

  McFlibError = dpio_destroy(DprcMcIo, MC_CMD_NO_FLAGS, Dpaa2NetInterface->DpioHandle);
  if (McFlibError != 0) {
      DPAA2_ERROR_MSG("Error calling dpio_destroy() for 0x%p\n",
                      Dpaa2NetInterface);
  }

  Dpaa2NetInterface->DpioHandle = 0;
  DPAA2_DEBUG_MSG("Destroyed dpio.%u\n", Dpaa2NetInterface->DpioId);
}
  ...
  )
{
  ...
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpni_prepare_extended_cfg() for 0x%p\n",
                    Dpaa2NetInterface);
    return EFI_DEVICE_ERROR;
  }
  ...
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpni_create() for 0x%p\n",
                    Dpaa2NetInterface);
    return EFI_DEVICE_ERROR;
  }
  ...
  McFlibError = dpni_get_attributes(DprcMcIo, MC_CMD_NO_FLAGS,
				    Dpaa2NetInterface->DpniHandle,
                                    &DpniAttr);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpni_get_attributes() for 0x%p\n",
                    Dpaa2NetInterface);
    Status = EFI_DEVICE_ERROR;
    goto ErrorExitDestroyDpni;
  }

  if (gDpaa2DebugFlags & DPAA2_DEBUG_EXTRA_CHECKS) {
  ...
  DPAA2_DEBUG_MSG("Created dpni.%u in the MC\n", DpniAttr.id);
  return EFI_SUCCESS;
  ...
  )
{
  ...
  if (McFlibError != 0) {
      DPAA2_ERROR_MSG("Error calling dpni_destroy() for 0x%p\n",
                      Dpaa2NetInterface);
  }

  Dpaa2NetInterface->DpniHandle = 0;
  DPAA2_DEBUG_MSG("Destroyed dpni.%u\n", Dpaa2NetInterface->DpniId);
}
  ...
  )
{
  ...
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpmac_create() for 0x%p\n",
                    Dpaa2NetInterface);
    return EFI_DEVICE_ERROR;
  }
  ...
  McFlibError = dpmac_get_attributes(DprcMcIo, MC_CMD_NO_FLAGS,
				     Dpaa2NetInterface->DpmacHandle,
                                     &DpmacAttr);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpmac_get_attributes() for 0x%p\n",
                    Dpaa2NetInterface);
    Status = EFI_DEVICE_ERROR;
  ...
  DPAA2_DEBUG_MSG("Created dpmac.%u in the MC\n", DpmacAttr.id);
  return EFI_SUCCESS;
  ...
  )
{
  ...
  if (McFlibError != 0) {
      DPAA2_ERROR_MSG("Error calling dpmac_destroy() for 0x%p\n",
                      Dpaa2NetInterface);
  }

  Dpaa2NetInterface->DpmacHandle = 0;
  DPAA2_DEBUG_MSG("Destroyed dpmac.%u\n", Dpaa2NetInterface->DpmacId);
}
 ...
 **/
EFI_STATUS
Dpaa2McCreateNetworkInterface(DPAA2_NETWORK_INTERFACE *Dpaa2NetInterface)
{
  EFI_STATUS Status;
  DPAA2_MANAGEMENT_COMPLEX *Mc = &gManagementComplex;
  ...
  DPAA2_NETWORK_INTERFACE *Dpaa2NetInterface)
{
  ...
  McFlibError = dprc_connect(DprcMcIo,
			     ...
			     &DprcConnectionCfg);
  if (McFlibError != 0) {
      DPAA2_ERROR_MSG("Error calling dprc_connect() for 0x%p\n",
                      Dpaa2NetInterface);
      return EFI_DEVICE_ERROR;
  }
...
{
  INT32 McFlibError;
  UINT16 DpniHandle = Dpaa2NetInterface->DpniHandle;
  ...
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpni_set_rx_buffer_layout() for 0x%p\n",
                    Dpaa2NetInterface);
    return EFI_DEVICE_ERROR;
  }
  ...
  McFlibError = dpni_set_tx_buffer_layout(DprcMcIo, MC_CMD_NO_FLAGS,
					  DpniHandle, &DpniBufLayout);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpni_set_tx_buffer_layout() for 0x%p\n",
                    Dpaa2NetInterface);
    return EFI_DEVICE_ERROR;
  }
  ...
  McFlibError = dpni_set_tx_conf_buffer_layout(DprcMcIo, MC_CMD_NO_FLAGS,
                                               DpniHandle, &DpniBufLayout);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpni_set_tx_conf_buffer_layout() for 0x%p\n",
                    Dpaa2NetInterface);
    return EFI_DEVICE_ERROR;
  }
  ...
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpni_get_tx_data_offset() for 0x%p\n",
                    Dpaa2NetInterface);
    return EFI_DEVICE_ERROR;
  }

  if (Dpaa2NetInterface->TxDataOffset % 64 != 0) {
    DPAA2_WARN_MSG("WARNING: Tx data offset is not a multiple of 64 for dpni.%u\n",
                   Dpaa2NetInterface->DpniId);
  }
  ...
  )
{
  ...
  for (I = 0; I < 7; I++) {
    ...
    if (DataBuffer == NULL) {
      DPAA2_ERROR_MSG("Could not allocate DPAA2 Ethernet frame buffer\n");
      goto ErrorAlloc;
    }
...
ReleaseBuffers:
  ...
  DPAA2_DEBUG_MSG("Added %u buffers to QBMAN buffer pool %u\n",
                  I, HwBufferPoolId);
  return I;
...
C_ASSERT(DPAA2_ETH_NUM_FRAME_BUFFERS % 7 == 0);
...
static
EFI_STATUS
Dpaa2QbmanSeedBufferPool(DPAA2_QBMAN_PORTAL *DpioQbmanPortal,
                         UINT16 HwBufferPoolId)
{
  UINT32 I;
  UINT8 BufCount;
  ...
  DPAA2_NETWORK_INTERFACE *Dpaa2NetInterface)
{
  ...
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpbp_enable() for 0x%p\n",
                    Dpaa2NetInterface);
    return EFI_DEVICE_ERROR;
  }
  ...
  UINT32 NumBuffers)
{
  ...
  DPAA2_DEBUG_MSG("Removing %u buffers from QBMAN buffer pool %u ...\n",
  ...
  for (NumRemainingBuffers = NumBuffers;
    ...
    if (NumAcquiredBuffers == 0) {
      DPAA2_ERROR_MSG("Could not acquire %u buffers from QBman buffer pool %u\n",
    ...
    if(J < I) {
      DPAA2_WARN_MSG("Duplicate buffer addresses received from QBman buffer pool %u\n",
        HwBufferPoolId);
      continue;
    }
  ...
  DPAA2_NETWORK_INTERFACE *Dpaa2NetInterface)
{
  ...
  McFlibError = dpni_set_pools(DprcMcIo, MC_CMD_NO_FLAGS,
			       Dpaa2NetInterface->DpniHandle,
                               &PoolsParams);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpni_set_pools() for 0x%p\n",
                    Dpaa2NetInterface);
    return EFI_DEVICE_ERROR;
  }
  ...
  McFlibError = dpni_set_tx_flow(DprcMcIo, MC_CMD_NO_FLAGS,
			         ...
			         &TxFlowCfg);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpni_set_tx_flow() for 0x%p\n",
                    Dpaa2NetInterface);
    return EFI_DEVICE_ERROR;
  }
  ...
  McFlibError = dpni_set_tx_conf(DprcMcIo, MC_CMD_NO_FLAGS,
			         ...
			         Dpaa2NetInterface->TxFlowId,
                                 &TxConfCfg);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpni_set_tx_conf() for 0x%p\n",
                    Dpaa2NetInterface);
    return EFI_DEVICE_ERROR;
  }
  ...
  McFlibError = dpni_get_tx_conf(DprcMcIo, MC_CMD_NO_FLAGS,
			         ...
			         &TxConfAttr);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpni_get_tx_conf() for 0x%p\n",
                    Dpaa2NetInterface);
    return EFI_DEVICE_ERROR;
  }
  ...
  EFI_MAC_ADDRESS *MacAddr)
{
  ...
  McFlibError = dpni_add_mac_addr(DprcMcIo, MC_CMD_NO_FLAGS,
				  Dpaa2NetInterface->DpniHandle,
                                  MacAddr->Addr);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpni_add_mac_addr() for 0x%p\n",
                    Dpaa2NetInterface);
    return EFI_DEVICE_ERROR;
  }
  ...
  McFlibError = dpni_enable(DprcMcIo, MC_CMD_NO_FLAGS,
                            Dpaa2NetInterface->DpniHandle);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpni_enable() for 0x%p\n",
                    Dpaa2NetInterface);
    return EFI_DEVICE_ERROR;
  }
  ...
  WRIOP_DPMAC *WriopDpmac)
{
  ...
  McFlibError = dpmac_set_link_state(DprcMcIo, MC_CMD_NO_FLAGS,
                                     ...
                                     &DpmacLinkState);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpmac_set_link_state() for 0x%p\n",
                    Dpaa2NetInterface);
    return EFI_DEVICE_ERROR;
  }
  ...
  DPAA2_NETWORK_INTERFACE *Dpaa2NetInterface)
{
  ...
  McFlibError = dpni_get_link_state(&Dpaa2NetInterface->DprcMcIo,
				    ...
				    Dpaa2NetInterface->DpniHandle,
                                    &DpniLinkState);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpni_get_link_state() for 0x%p\n",
                    Dpaa2NetInterface);
    return FALSE;
  }
  ...
  DPAA2_NETWORK_INTERFACE *Dpaa2NetInterface)
{
  ...
  McFlibError = dpni_get_rx_flow(DprcMcIo, MC_CMD_NO_FLAGS,
			         Dpaa2NetInterface->DpniHandle,
                                 0, 0, &RxQueueAttr);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpni_get_rx_flow() for 0x%p\n",
                    Dpaa2NetInterface);
    Status = EFI_DEVICE_ERROR;
  ...
  if (McFlibError) {
    DPAA2_ERROR_MSG("Error calling dpni_get_qdid() for 0x%p\n",
                    Dpaa2NetInterface);
    Status = EFI_DEVICE_ERROR;
    goto ErrorExitDisableDpni;
  }
...
)
{
  ...
  *Mtu -= DPAA2_ETHERNET_HEADER_SIZE;
  if (McFlibError) {
    DPAA2_ERROR_MSG("Error calling dpni_get_mtu() for 0x%p\n",
                    Dpaa2NetInterface);
    return EFI_DEVICE_ERROR;
  }
  return EFI_SUCCESS;
  ...
  WRIOP_DPMAC *WriopDpmac)
{
  ...
  if (gDpaa2DebugFlags & DPAA2_DEBUG_EXTRA_CHECKS) {
    if (!Dpaa2McCheckDpniLink(Dpaa2NetInterface)) {
      DPAA2_DEBUG_MSG("DPNI link is not up for 0x%p\n", Dpaa2NetInterface);
    }
  }
  ...
  )
{
  ...
  McFlibError = dpni_get_counter(DprcMcIo, MC_CMD_NO_FLAGS,
		                 ...
		                 &Value);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpni_get_counter(DPNI_CNT_ING_FRAME) for 0x%p\n",
                    Dpaa2NetInterface);
    return;
  }

  DPAA2_DEBUG_MSG("DPNI_CNT_ING_FRAME=%llu\n", Value);
  ...
  McFlibError = dpni_get_counter(DprcMcIo, MC_CMD_NO_FLAGS,
		                 ...
		                 &Value);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpni_get_counter(DPNI_CNT_ING_BYTE) for 0x%p\n",
                    Dpaa2NetInterface);
    return;
  }

  DPAA2_DEBUG_MSG("DPNI_CNT_ING_BYTE=%llu\n", Value);
  ...
  McFlibError = dpni_get_counter(DprcMcIo, MC_CMD_NO_FLAGS,
		                 ...
		                 &Value);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpni_get_counter(DPNI_CNT_ING_FRAME_DROP) for 0x%p\n",
                    Dpaa2NetInterface);
    return;
  }

  DPAA2_DEBUG_MSG("DPNI_CNT_ING_FRAME_DROP=%llu\n", Value);
  ...
  McFlibError = dpni_get_counter(DprcMcIo, MC_CMD_NO_FLAGS,
		                 ...
		                 DPNI_CNT_ING_FRAME_DISCARD,
                                 &Value);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpni_get_counter(DPNI_CNT_ING_FRAME_DISCARD) for 0x%p\n",
                    Dpaa2NetInterface);
    return;
  }

  DPAA2_DEBUG_MSG("DPNI_CNT_ING_FRAME_DISCARD=%llu\n", Value);
  ...
  McFlibError = dpni_get_counter(DprcMcIo, MC_CMD_NO_FLAGS,
		                 ...
		                 DPNI_CNT_EGR_FRAME,
                                 &Value);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpni_get_counter(DPNI_CNT_EGR_FRAME) for 0x%p\n",
                    Dpaa2NetInterface);
    return;
  }

  DPAA2_DEBUG_MSG("DPNI_CNT_EGR_FRAME=%llu\n", Value);
  ...
  McFlibError = dpni_get_counter(DprcMcIo, MC_CMD_NO_FLAGS,
		                 ...
		                 DPNI_CNT_EGR_BYTE,
                                 &Value);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpni_get_counter(DPNI_CNT_EGR_BYTE) for 0x%p\n",
                    Dpaa2NetInterface);
    return;
  }

  DPAA2_DEBUG_MSG("DPNI_CNT_EGR_BYTE=%llu\n", Value);
  ...
  McFlibError = dpni_get_counter(DprcMcIo, MC_CMD_NO_FLAGS,
		                 ...
		                 DPNI_CNT_EGR_FRAME_DISCARD,
                                 &Value);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpni_get_counter(DPNI_CNT_EGR_FRAME_DISCARD) for 0x%p\n",
                    Dpaa2NetInterface);
    return;
  }

  DPAA2_DEBUG_MSG("DPNI_CNT_EGR_FRAME_DISCARD=%llu\n", Value);
}
  ...
  )
{
  ...
  McFlibError = dpmac_get_counter(DprcMcIo, MC_CMD_NO_FLAGS,
		                  ...
		                  &Value);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpmac_get_counter(DPMAC_CNT_ING_BYTE) for 0x%p\n",
                    Dpaa2NetInterface);
    return;
  }

  DPAA2_DEBUG_MSG("DPMAC_CNT_ING_BYTE=%llu\n", Value);
  ...
  McFlibError = dpmac_get_counter(DprcMcIo, MC_CMD_NO_FLAGS,
		                  ...
		                  DPMAC_CNT_ING_FRAME_DISCARD,
                                  &Value);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpmac_get_counter(DPMAC_CNT_ING_FRAME_DISCARD) for 0x%p\n",
                    Dpaa2NetInterface);
    return;
  }

  DPAA2_DEBUG_MSG("DPMAC_CNT_ING_FRAME_DISCARD=%llu\n", Value);
  ...
  McFlibError = dpmac_get_counter(DprcMcIo, MC_CMD_NO_FLAGS,
		                  ...
		                  &Value);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpmac_get_counter(DPMAC_CNT_ING_ALIGN_ERR) for 0x%p\n",
                    Dpaa2NetInterface);
    return;
  }

  DPAA2_DEBUG_MSG("DPMAC_CNT_ING_ALIGN_ERR=%llu\n", Value);
  ...
  McFlibError = dpmac_get_counter(DprcMcIo, MC_CMD_NO_FLAGS,
		                  ...
		                  &Value);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpmac_get_counter(DPMAC_CNT_ING_ERR_FRAME) for 0x%p\n",
                    Dpaa2NetInterface);
    return;
  }

  DPAA2_DEBUG_MSG("DPMAC_CNT_ING_ERR_FRAME=%llu\n", Value);
  ...
  McFlibError = dpmac_get_counter(DprcMcIo, MC_CMD_NO_FLAGS,
		                  ...
		                  DPMAC_CNT_EGR_BYTE,
                                  &Value);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpmac_get_counter(DPMAC_CNT_EGR_BYTE) for 0x%p\n",
                    Dpaa2NetInterface);
    return;
  }

  DPAA2_DEBUG_MSG("DPMAC_CNT_EGR_BYTE=%llu\n", Value);
  ...
  McFlibError = dpmac_get_counter(DprcMcIo, MC_CMD_NO_FLAGS,
		                  ...
		                  DPMAC_CNT_EGR_ERR_FRAME,
                                  &Value);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpmac_get_counter(DPMAC_CNT_EGR_ERR_FRAME) for 0x%p\n",
                    Dpaa2NetInterface);
    return;
  }

  DPAA2_DEBUG_MSG("DPMAC_CNT_EGR_ERR_FRAME=%llu\n", Value);
}
  ...
  DPAA2_NETWORK_INTERFACE *Dpaa2NetInterface)
{
  ...
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dprc_disconnect() for 0x%p\n",
                    Dpaa2NetInterface);
  }
}
  ...
  DPAA2_NETWORK_INTERFACE *Dpaa2NetInterface)
{
  ...
  McFlibError = dpni_disable(DprcMcIo, MC_CMD_NO_FLAGS,
                             Dpaa2NetInterface->DpniHandle);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpni_disable() for 0x%p\n",
                    Dpaa2NetInterface);
  }
}
  ...
  DPAA2_NETWORK_INTERFACE *Dpaa2NetInterface)
{
  ...
  if (gDpaa2DebugFlags & DPAA2_DEBUG_DUMP_VALUES) {
...
# define QBMAN_TX_BUFFER_ENQUEUE_TIMEOUT_US 2000
  ...
  UINT32 TimeoutCount;
  UINT32 NumAcquiredBuffers;
  UINT16 DataOffset = Dpaa2NetInterface->TxDataOffset;
  DPAA2_QBMAN_PORTAL *DpioQbmanPortal = &Dpaa2NetInterface->DpioQbmanPortal;
  UINT16 HwBufferPoolId = Dpaa2NetInterface->DpbpHwBufferPoolId;
  ...
  for ( ; ; ) {
    ...
    if (TimeoutCount == 0) {
      DPAA2_ERROR_MSG("Timeout enqueueing Tx buffer to QBman for interface 0x%p\n",
                      Dpaa2NetInterface);
  ...
  if (gDpaa2DebugFlags & DPAA2_DEBUG_TRACE_NET_PACKETS) {
    DPAA2_DEBUG_MSG("Tx: %x:%x:%x:%x:%x:%x|%x:%x:%x:%x:%x:%x|%x|%x|%x%x%x%x\n",
...
ErrorExit:
  ...
  for ( ; ; ) {
    ...
    if (TimeoutCount == 0) {
      DPAA2_ERROR_MSG("Timeout releasing Tx buffer to QBman for interface 0x%p\n",
                      Dpaa2NetInterface);
      break;
    }
...
)
{
  ...
  if (FrameDesc->Simple.FrameContext & QBMAN_FD_FRC_FASV) {
    ...
    if (FrameStatus & QBMAN_ETH_RX_ERROR_MASK) {
      DPAA2_ERROR_MSG("Frame received with errors: 0x%08x\n",
		      FrameStatus & QBMAN_ETH_RX_ERROR_MASK);
      goto CommonExit;
    }

    if (FrameStatus & QBMAN_ETH_RX_UNSUPPORTED_MASK) {
      DPAA2_ERROR_MSG("Frame received with unsupported features: 0x%08x\n",
  ...
  if (*BuffSize < FrameDataLength) {
    DPAA2_ERROR_MSG("Frame truncated (actual frame length %u, buffer size: %lu\n",
...
CommonExit:
  *BuffSize = DataSize;
  ...
  for ( ; ; ) {
    ...
    if (TimeoutCount == 0) {
      DPAA2_ERROR_MSG("Timeout releasing Rx buffer to QBman for interface 0x%p\n",
                      Dpaa2NetInterface);
      break;
    }
...
)
{
  ...
  for ( ; ; ) {
    ...
    if (TimeoutCount == 0) {
      DPAA2_ERROR_MSG("Timeout dequeueing Rx frame from QBman for interface 0x%p\n",
                      Dpaa2NetInterface);
      return EFI_NOT_READY;
    }
  ...
  if (gDpaa2DebugFlags & DPAA2_DEBUG_TRACE_NET_PACKETS) {
    DPAA2_DEBUG_MSG("Rx: %x:%x:%x:%x:%x:%x|%x:%x:%x:%x:%x:%x|%x|%x|%x%x%x%x\n",
      SrcAddr->Addr[0], SrcAddr->Addr[1], SrcAddr->Addr[2],
      SrcAddr->Addr[3], SrcAddr->Addr[4], SrcAddr->Addr[5],
      DstAddr->Addr[0], DstAddr->Addr[1], DstAddr->Addr[2],
      DstAddr->Addr[3], DstAddr->Addr[4], DstAddr->Addr[5],
  ...
  EFI_MAC_ADDRESS *MulticastMacAddr)
{
  ...
  DPAA2_DEBUG_MSG("Added multicast address %x:%x:%x:%x:%x:%x to dpni.%u\n",
  ...
  EFI_MAC_ADDRESS *MulticastMacAddr)
{
  ...
  McFlibError = dpni_remove_mac_addr(&Dpaa2NetInterface->DprcMcIo,
                                     ...
                                     MulticastMacAddr->Addr);
  if (McFlibError != 0) {
    DPAA2_ERROR_MSG("Error calling dpni_remove_mac_addr() for 0x%p\n",
                    Dpaa2NetInterface);
    return EFI_DEVICE_ERROR;
  }

  DPAA2_DEBUG_MSG("Removed multicast address %x:%x:%x:%x:%x:%x to dpni.%u\n",
                  MulticastMacAddr->Addr[0],
                  MulticastMacAddr->Addr[1],
                  MulticastMacAddr->Addr[2],
                  MulticastMacAddr->Addr[3],
...
)
{
  ...
  for ( ; ; ) {
    ...
    if (TimeoutCount == 0) {
      DPAA2_ERROR_MSG("Timeout dequeueing Tx confirmation from QBman for interface 0x%p\n",
                      Dpaa2NetInterface);
      return EFI_NOT_READY;
    }
