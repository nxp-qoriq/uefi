...
C_ASSERT(sizeof(unsigned int) == sizeof(UINT32));
...
C_ASSERT(CACHELINE_SIZE / sizeof(UINT32) == 16);

static
inline
VOID ZerorCacheLine(UINT32 *Ptr)
{
...
{
  UINT32 *Shadow =
    (UINT32 *)((UINT8 *)QbmanPortal->CacheEnabledRegsShadow + Offset);
 ...
 **/
EFI_STATUS
QbmanSwPortalInit(DPAA2_QBMAN_PORTAL *DpioQbmanPortal,
                  ...
                  UINT64 qbman_portal_ci_offset)
{
  ...
  DpioQbmanPortal->CacheEnabledBaseAddr =
    DPAA2_QBMAN_PORTALS_BASE_ADDR + qbman_portal_ce_offset;
  DpioQbmanPortal->CacheInhibitedBaseAddr =
    DPAA2_QBMAN_PORTALS_BASE_ADDR + qbman_portal_ci_offset;
  ...
  if (DpioQbmanPortal->CacheEnabledRegsShadow == NULL) {
    DPAA2_ERROR_MSG("Failed to allocate QBman's portal cache-enabled registers shadow\n");
  ...
  if (RegValue != 0) {
    DPAA2_ERROR_MSG("QBman software portal already configured\n");
  ...
  RegValue = QbmanCacheInhibitedRead(DpioQbmanPortal,
                                     QBMAN_PORTAL_CACHE_INHIBITED_CFG);
  if (RegValue == 0) {
    DPAA2_ERROR_MSG("QBman software portal could not be configured\n");
    Status = EFI_DEVICE_ERROR;
    goto ErrorExitFreeShadow;
  }
...
VOID
QbmanReleaseDescClear(QBMAN_RELEASE_DESC *QbmanReleaseDesc)
{
...
VOID
QbmanReleaseDescSetBpid(QBMAN_RELEASE_DESC *QbmanReleaseDesc,
...
VOID
QbmanEnqueueDescClear(QBMAN_ENQUEUE_DESC *QbmanEnqueueDesc)
{
...
VOID
QbmanEnqueueDescSetNoOrp(QBMAN_ENQUEUE_DESC *QbmanEnqueueDesc,
...
VOID
QbmanEnqueueDescSetQueueDest(QBMAN_ENQUEUE_DESC *QbmanEnqueueDesc,
                             UINT32 QueueDestId,
                             UINT32 QueueDestBin,
...
# define RAR_SUCCESS(rar) ((rar) & 0x100)
  ...
  UINT32 *CmdBuf;
  UINT32 *CacheLine = (UINT32 *)QbmanReleaseDesc;
...
static
UINT32 *
QbmanSwPortalCmdStart(DPAA2_QBMAN_PORTAL *QbmanPortal)
{
  UINT32 *CmdBuf;
...
static
UINT32 *
QbmanSwPortalCmdResult(DPAA2_QBMAN_PORTAL *QbmanPortal)
{
  UINT32 *ResultBuf;
  UINT32 Verb;
...
inline
UINT32 *
QbmanSwPortalCmdSubmitAndWait(DPAA2_QBMAN_PORTAL *QbmanPortal,
                              UINT32 *Cmd,
                              UINT32 CmdVerb)
{
  /**
...
# define QBMAN_WAIT_CMD_RESULT_TIMEOUT_US    2000
  ...
  for ( ; ; ) {
    ...
    if (TimeoutCount == 0) {
      DPAA2_ERROR_MSG("Timeout waiting for command result for QBman portal 0x%p\n",
                      QbmanPortal);
      QbmanPortal->MgmtCmd.State = QBMAN_PORTAL_CMD_CAN_START;
      break;
    }
...
# define EQAR_SUCCESS(eqar) ((eqar) & 0x100)
  ...
  UINT32 *CmdBuf;
  UINT32 *CacheLine = (UINT32 *)QbmanEnqueueDesc;
  UINT32 Eqar = QbmanCacheInhibitedRead(QbmanPortal,
                                        QBMAN_PORTAL_CACHE_INHIBITED_EQAR);
...
static
VOID
AtomicInt32Increment(volatile INT32 *Var)
{
...
static
BOOLEAN
AtomicInt32DecrementAndTest(volatile INT32 *Var)
{
  UINTN SavedInterruptState;
  INT32 Value;
