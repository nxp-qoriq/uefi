...
C_ASSERT(ARRAY_SIZE(gPhyMediaTypeStrings) == NUM_PHY_MEDIA_TYPES);
 ...
 **/
static EFI_STATUS
Dpaa2PhyMdioBusInit(DPAA2_PHY_MDIO_BUS *MdioBus)
{
  MEMAC_MDIO_BUS_REGS *const MdioBusRegs = MdioBus->IoRegs;

  UINT32 RegValue = MmioRead32((UINTN)&MdioBusRegs->MdioStat);
 ...
 **/
EFI_STATUS
Dpaa2PhyMdioBusesInit(DPAA2_PHY_MDIO_BUS MdioBuses[], UINT8 MdioBusesCount)
{
  EFI_STATUS Status;
  UINT8 I;
  ...
  IN UINT16 PhyRegNum)
{
  ...
  if (RegValue & MDIO_STAT_RD_ER) {
     ...
     */
    DPAA2_ERROR_MSG("MDIO bus read failed for PHY addr 0x%x, dev addr 0x%x, "
...
static
EFI_STATUS
Dpaa2PhyReset(DPAA2_PHY *Dpaa2Phy)
{
  UINT16 PhyRegValue;
  ...
  DPAA2_DEBUG_MSG("Resetting PHY (PHY address: 0x%x) ...\n",
                  Dpaa2Phy->PhyAddress);
  ...
  for ( ; ; ) {
    ...
    if (TimeoutMsCount == 0) {
      DPAA2_ERROR_MSG("Timeout resetting PHY (PHY address: 0x%x)\n",
                      Dpaa2Phy->PhyAddress);
...
static
EFI_STATUS
Dpaa2PhyConfig(DPAA2_PHY *Dpaa2Phy)
{
  DPAA2_DEBUG_MSG("Configuring PHY (PHY address: 0x%x) ...\n",
                  Dpaa2Phy->PhyAddress);
  ...
  if (Dpaa2Phy->PhyMediaType == COPPER_PHY) {
    ...
    DPAA2_ERROR_MSG("PHY media type not supported: 0x%x\n",
 ...
 **/
EFI_STATUS
Dpaa2PhyInit(DPAA2_PHY *Dpaa2Phy)
{
  EFI_STATUS Status;

  DPAA2_DEBUG_MSG("Initializing PHY (PHY address: 0x%x) ...\n",
                  Dpaa2Phy->PhyAddress);
 ...
 **/
EFI_STATUS
Dpaa2PhyStartup(DPAA2_PHY *Dpaa2Phy)
{
  DPAA2_INFO_MSG("Starting up PHY (PHY address: 0x%x) ...\n",
                 Dpaa2Phy->PhyAddress);
  ...
  if (Dpaa2Phy->PhyMediaType == COPPER_PHY) {
    ...
    DPAA2_ERROR_MSG("PHY media type not supported: 0x%x (%a)\n",
                    Dpaa2Phy->PhyMediaType,
 ...
 **/
VOID
Dpaa2PhyShutdown(DPAA2_PHY *Dpaa2Phy)
{
  /*
