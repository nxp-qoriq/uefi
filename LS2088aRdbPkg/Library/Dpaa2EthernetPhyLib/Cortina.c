...
static
EFI_STATUS
LoadPhyFirmware(DPAA2_PHY *Dpaa2Phy)
{
# define TEXT_LINE_SIZE  80

  EFI_PHYSICAL_ADDRESS FwFlashAddr;
  UINT8 FwSrc = PcdGet8(PcdDpaa2CortinaFwSrc);
  UINT32 FwMaxLines = PcdGet32(PcdDpaa2CortinaFwMaxLen);
  ...
  if (FwSrc == 0x1) {
    FwFlashAddr = PcdGet64(PcdDpaa2CortinaFwNorAddr);
    DPAA2_INFO_MSG(
      "Loading Cortina firmware from NOR flash address 0x%p for PHY address 0x%x ...\n",
      FwFlashAddr, Dpaa2Phy->PhyAddress);
  } else {
    DPAA2_ERROR_MSG("Storage media for Cortina firmware image not supported: 0x%x\n",
                    FwSrc);
  ...
  while (*FwFlashPtr != 'Q') {
    ...
    while (*FwFlashPtr != '\n') {
      if (I >= TEXT_LINE_SIZE - 1) {
          DPAA2_ERROR_MSG("Line %d in Cortina Firmware is too long\n",
                          LineCount + 1);
          return EFI_INVALID_PARAMETER;
      }
    ...
    if (LineCount > FwMaxLines) {
      DPAA2_ERROR_MSG("Cortina Firmware larger than expected\n");
      return EFI_INVALID_PARAMETER;
    }

    TokenStartPtr = SkipSeparators(LineTemp);
    if (*TokenStartPtr == '\0') {
      DPAA2_ERROR_MSG("Invalid blank line found in Cortina Firmware\n");
      return EFI_INVALID_PARAMETER;
    }
    ...
    if (*TokenStartPtr == '\0') {
      DPAA2_ERROR_MSG("Incomplete line found in Cortina Firmware\n");
      return EFI_INVALID_PARAMETER;
    }
 ...
 **/
EFI_STATUS
CortinaPhyConfig(DPAA2_PHY *Dpaa2Phy)
{
  /**
...
# define BIST_TEST_DONE_TIMEOUT_MS    100
  ...
  for ( ; ; ) {
    ...
    if (TimeoutCount == 0) {
      DPAA2_ERROR_MSG("BIST mseq_edc_bist_done timeout for Cortina PHY 0x%p\n",
                      Dpaa2Phy);
  ...
  if (PhyRegValue != 0) {
      DPAA2_ERROR_MSG("Firmware checksum status failed (0x%x) for Cortina PHY 0x%p\n",
 ...
 **/
EFI_STATUS
CortinaPhyStartup(DPAA2_PHY *Dpaa2Phy)
{
  ASSERT(Dpaa2Phy->Signature == DPAA2_PHY_SIGNATURE);
