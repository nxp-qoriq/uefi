 ...
 **/
EFI_STATUS
AquantiaPhyConfig(DPAA2_PHY *Dpaa2Phy)
{
  UINT16 PhyRegValue;
 ...
 **/
EFI_STATUS
AquantiaPhyStartup(DPAA2_PHY *Dpaa2Phy)
{
  UINTN I;
  UINT16 PhyRegValue;
  ...
  if (!(PhyRegValue & PHY_STATUS_AUTO_NEGOTIATION_COMPLETE)) {
    DPAA2_DEBUG_MSG("Waiting for PHY (PHY address: 0x%x) auto negotiation to complete ",
                    Dpaa2Phy->PhyAddress);
    ...
    if (I == PHY_AUTO_NEGOTIATION_TIMEOUT) {
      DPAA2_DEBUG_MSG_NO_PREFIX("TIMEOUT!\n");
      DPAA2_ERROR_MSG("PHY auto-negotiation failed\n");
