  ...
  do {
    ...
    if (!Status) {
      if ((Cmd.Response[0] & MMC_STATUS_RDY_FOR_DATA) &&
        ...
        DEBUG((EFI_D_ERROR, "SendStatus Error: 0x%08X\n", Cmd.Response[0]));
...
RetryScr:
  ...
  Mmc->Scr[0] = Uswap32(Scr[0]);
  Mmc->Scr[1] = Uswap32(Scr[1]);
  ...
  do {
    ...
    if (!Uswap32((SwitchStatus[7]) & SD_HIGHSPEED_BUSY))
      break;

  }while (Timeout--);

  /* Return if High-Speed Isn'T Supported */
  if (!(Uswap32(SwitchStatus[3]) & SD_HIGHSPEED_SUPPORTED)) {
    Status = 0;
    goto FreeMem;
  }
  ...
  if ((Uswap32(SwitchStatus[4]) & 0x0f000000) == 0x01000000)
  ...
  )
{
  ...
  */
  ...
  /* Dummy read required for erase to work */
  UINT8 Buffer[Mmc->BlockDev.Blksz];
  SdxcBlkRead(0, 1, Buffer);
