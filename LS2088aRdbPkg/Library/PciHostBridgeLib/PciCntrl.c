...
{
	if (Num == 1) {
	PcieInfo->Regs = PCIE1_BASE_ADDR;
	PcieInfo->Cfg0Phys = LS_PCIE_CFG0_PHYS_OFF +
		      		PCIE1_PHYS_ADDR;
	PcieInfo->Cfg0Size = LS_PCIE_CFG0_SIZE;
	PcieInfo->Cfg1Phys = LS_PCIE_CFG1_PHYS_OFF +
		      		PCIE1_PHYS_ADDR;
	PcieInfo->Cfg1Size = LS_PCIE_CFG1_SIZE;
	PcieInfo->MemBus = LS_PCIE_MEM_BUS;
	PcieInfo->MemPhys = LS_PCIE_MEM_PHYS_OFF +
		     		PCIE1_PHYS_ADDR;
	PcieInfo->MemSize = LS_PCIE_MEM_SIZE;
	PcieInfo->IoBus = LS_PCIE_IO_BUS;
	PcieInfo->IoPhys = LS_PCIE_IO_PHYS_OFF +
		    		PCIE1_PHYS_ADDR;
	PcieInfo->IoSize = LS_PCIE_IO_SIZE;
	PcieInfo->PciNum = Num;
	} else if (Num == 2) {
	PcieInfo->Regs = PCIE2_BASE_ADDR;
	PcieInfo->Cfg0Phys = LS_PCIE_CFG0_PHYS_OFF +
		      		PCIE2_PHYS_ADDR;
	PcieInfo->Cfg0Size = LS_PCIE_CFG0_SIZE;
	PcieInfo->Cfg1Phys = LS_PCIE_CFG1_PHYS_OFF +
...
)	
{
	UINTN Dev = ((PrivateData->FirstBusno) << 16 | (0) << 11 | (0) << 8);
        ...
        if (FeaturePcdGet(PcdPciDebug) == TRUE)
	  DEBUG((EFI_D_INFO, "Setting Up IATU\n\n"));
