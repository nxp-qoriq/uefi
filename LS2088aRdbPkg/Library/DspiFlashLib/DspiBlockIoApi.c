  ...
  )
{
  ...
  //Read block
  SelectMuxDevice(MUX_DSPI);

  Status = DspiFlashRead((UINTN)Lba, BufferSize, Buffer);
  if (EFI_ERROR(Status)) {
    DEBUG((EFI_D_ERROR, "Read block fails: %x\n", Status));
    return Status;
  }

  DEBUG((EFI_D_RELEASE,"DSPI : %d bytes read %a\n",
...
)
{
  ...
  Status = DspiFlashErase((UINTN)Lba, BufferSize);
  if (EFI_ERROR(Status)) {
    DEBUG((EFI_D_ERROR, "Block Erase fails: %x\n", Status));
    return Status;
  }
  DEBUG((EFI_D_RELEASE,"DSPI : %d bytes erased %a\n",
		BufferSize, Status ? "ERROR" : "OK"));
...
)
{
  ...
  Status = DspiFlashWrite((UINTN)Lba, BufferSize, Buffer);
  if (EFI_ERROR(Status)) {
    DEBUG((EFI_D_ERROR, "Block write fails: %x\n", Status));
    return Status;
  }
  DEBUG((EFI_D_RELEASE,"DSPI : %d bytes written %a\n",
		BufferSize, Status ? "ERROR" : "OK"));
