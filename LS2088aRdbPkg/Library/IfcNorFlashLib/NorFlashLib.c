  ...
  )
{
  ...
  SEND_NOR_COMMAND(Instance->DeviceBaseAddress, S29GL01GP_CMD_UNLOCK_1_ADDR, S29GL01GP_CMD_SECTOR_ERASE_FIRST);

  SEND_NOR_COMMAND(Instance->DeviceBaseAddress, S29GL01GP_CMD_UNLOCK_2_ADDR, S29GL01GP_CMD_SECTOR_ERASE_SECOND);

  // Issue a setup command
  SEND_NOR_COMMAND(Instance->DeviceBaseAddress, S29GL01GP_CMD_UNLOCK_1_ADDR, S29GL01GP_CMD_SECTOR_ERASE_THIRD);

  // Issue the Unlock cmds
  SEND_NOR_COMMAND(Instance->DeviceBaseAddress, S29GL01GP_CMD_UNLOCK_1_ADDR, S29GL01GP_CMD_SECTOR_ERASE_FOURTH);
  
  SEND_NOR_COMMAND(Instance->DeviceBaseAddress, S29GL01GP_CMD_UNLOCK_2_ADDR, S29GL01GP_CMD_SECTOR_ERASE_FIFTH);

  // Now send the address of the sector to be erased
  SEND_NOR_COMMAND(SectorAddress, 0, S29GL01GP_CMD_SECTOR_ERASE_SIXTH);
  ...
  )
{
  ...
  // Issue the Unlock cmds
  SEND_NOR_COMMAND(Instance->DeviceBaseAddress, S29GL01GP_CMD_UNLOCK_1_ADDR, S29GL01GP_CMD_CHIP_ERASE_FIRST);
  
  SEND_NOR_COMMAND(Instance->DeviceBaseAddress, S29GL01GP_CMD_UNLOCK_2_ADDR, S29GL01GP_CMD_CHIP_ERASE_SECOND);

  // Issue the setup cmd
  SEND_NOR_COMMAND(Instance->DeviceBaseAddress, S29GL01GP_CMD_UNLOCK_1_ADDR, S29GL01GP_CMD_CHIP_ERASE_THIRD);

  // Issue the Unlock cmds
  SEND_NOR_COMMAND(Instance->DeviceBaseAddress, S29GL01GP_CMD_UNLOCK_1_ADDR, S29GL01GP_CMD_CHIP_ERASE_FOURTH);
  
  SEND_NOR_COMMAND(Instance->DeviceBaseAddress, S29GL01GP_CMD_UNLOCK_2_ADDR, S29GL01GP_CMD_CHIP_ERASE_FIFTH);

  // Issue the chip erase cmd
  SEND_NOR_COMMAND(Instance->DeviceBaseAddress, S29GL01GP_CMD_UNLOCK_1_ADDR, S29GL01GP_CMD_CHIP_ERASE_SIXTH);
...
)
{
  ...
  // Issue the Unlock cmds
  SEND_NOR_COMMAND(Instance->DeviceBaseAddress, S29GL01GP_CMD_UNLOCK_1_ADDR,
		   S29GL01GP_CMD_PROGRAM_FIRST);

  SEND_NOR_COMMAND(Instance->DeviceBaseAddress, S29GL01GP_CMD_UNLOCK_2_ADDR,
		   S29GL01GP_CMD_PROGRAM_SECOND);

  SEND_NOR_COMMAND(Instance->DeviceBaseAddress, S29GL01GP_CMD_UNLOCK_1_ADDR,
                   S29GL01GP_CMD_PROGRAM_THIRD);
...
)
{
  ...
  // Issue the Unlock cmds
  SEND_NOR_COMMAND(Instance->DeviceBaseAddress, S29GL01GP_CMD_UNLOCK_1_ADDR,
		   S29GL01GP_CMD_WRITE_TO_BUFFER_FIRST);
  
  SEND_NOR_COMMAND(Instance->DeviceBaseAddress, S29GL01GP_CMD_UNLOCK_2_ADDR,
		   S29GL01GP_CMD_WRITE_TO_BUFFER_SECOND);

  // Write the buffer load
  SEND_NOR_COMMAND(TargetAddress, 0, S29GL01GP_CMD_WRITE_TO_BUFFER_THIRD);
  ...
  SEND_NOR_COMMAND (TargetAddress, 0,
		    S29GL01GP_CMD_WRITE_TO_BUFFER_CONFIRM);
  ...
  )
{
  ...
  if ((TargetOffsetinBytes + *NumBytes) > Instance->Size) {
  ...
  )
{
  ...
  if ((TargetOffsetinBytes + BufferSizeInBytes) > Instance->Size) {
...
{
  SEND_NOR_COMMAND(Instance->DeviceBaseAddress, S29GL01GP_CMD_UNLOCK_1_ADDR, S29GL01GP_CMD_RESET_FIRST);

  SEND_NOR_COMMAND(Instance->DeviceBaseAddress, S29GL01GP_CMD_UNLOCK_2_ADDR, S29GL01GP_CMD_RESET_SECOND);

  SEND_NOR_COMMAND (Instance->DeviceBaseAddress, 0, S29GL01GP_CMD_RESET);
  return EFI_SUCCESS;
}
